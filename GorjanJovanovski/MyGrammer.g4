...
question returns[ResultNode]
	...
	| txt=STRING lbl=LABEL DELIMITER 'date'  	{$ResultNode = new QuestionNode($txt.text, $lbl.text, new StringType(), $txt.line)}
	;

computedquestion returns[ResultNode]
	: txt=STRING lbl=LABEL DELIMITER 'decimal' '=' exp=expr  	{$ResultNode = new ComputedQuestionNode($txt.text, $lbl.text, new DecimalType(), $txt.line, $exp.ExprNode)}
	...
	| txt=STRING lbl=LABEL DELIMITER 'money' '=' exp=expr  	{$ResultNode = new ComputedQuestionNode($txt.text, $lbl.text, new DecimalType(), $txt.line, $exp.ExprNode)}
	| txt=STRING lbl=LABEL DELIMITER 'currency' '=' exp=expr  	{$ResultNode = new ComputedQuestionNode($txt.text, $lbl.text, new DecimalType(), $txt.line, $exp.ExprNode)}
	| txt=STRING lbl=LABEL DELIMITER 'date' '=' exp=expr  	{$ResultNode = new ComputedQuestionNode($txt.text, $lbl.text, new StringType(), $txt.line, $exp.ExprNode)}
	;
...
expr returns[ExprNode]
	: bool=BOOLEAN 								{$ExprNode = new LiteralNode('true' == $bool.text)} 						
	| lbl=LABEL 								{$ExprNode = new LabelNode($lbl.text, $lbl.line)} 		
	| num=NUMBER 								{$ExprNode = new LiteralNode(parseInt($num.text))} 						
	| dec=DECIMAL 								{$ExprNode = new LiteralNode(parseFloat($dec.text))} 					
