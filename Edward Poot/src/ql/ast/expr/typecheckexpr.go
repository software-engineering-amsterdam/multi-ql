...
func typeCheckInvalidUnaryOperatorForOperands(unaryOperator UnaryOperator, typeChecker interfaces.TypeChecker, symbolTable interfaces.SymbolTable) {
	...
	case bool:
		if exprType != "unaryoperatorNot" {
			typeChecker.AddEncounteredErrorForCheckType("InvalidOperands", fmt.Errorf("Encountered invalid operation for bool operand: %s", exprType))
		}
	case int:
		if exprType != "unaryoperatorNeg" && exprType != "unaryoperatorPos" {
			typeChecker.AddEncounteredErrorForCheckType("InvalidOperands", fmt.Errorf("Encountered invalid operation for int operand: %s", exprType))
		}
	case string:
		// there are no unary operators on strings, so always return error
		typeChecker.AddEncounteredErrorForCheckType("InvalidOperands", fmt.Errorf("Encountered invalid operation for string operand: %s", exprType))
	}
}
