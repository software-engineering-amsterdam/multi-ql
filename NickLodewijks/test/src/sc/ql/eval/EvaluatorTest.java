...
public class EvaluatorTest
{
  ...
  {
    // Integer literal
    Assert.assertEquals(new NumberValue(1),
                        interpret("1"));
    Assert.assertEquals(new NumberValue(2),
                        interpret("2"));
    ...
    // String literal
    Assert.assertEquals(new StringValue("test1"),
                        interpret("\"test1\""));
    Assert.assertEquals(new StringValue("test2"),
                        interpret("\"test2\""));

    // +
    Assert.assertEquals(new NumberValue(3),
                        interpret("1+2"));
    Assert.assertEquals(new NumberValue(3),
                        interpret("(1+2)"));
  ...
  public void testOperatorPrecedence()
      throws IOException
  {
    Assert.assertEquals(new NumberValue(6),
                        interpret("(1+2)*2"));
    Assert.assertEquals(new NumberValue(5),
                        interpret("1+2*2"));

    Assert.assertEquals(new NumberValue(1),
                        interpret("1"));
    Assert.assertEquals(new NumberValue(0),
                        interpret("1-1"));
    Assert.assertEquals(new NumberValue(2),
                        interpret("1--1"));
    Assert.assertEquals(new NumberValue(1),
    ...
    Assert.assertEquals(new NumberValue(-8),
                        interpret("-1 * (4*2)"));
    Assert.assertEquals(new NumberValue(-2),
                        interpret("-1 * (4/2)"));
    Assert.assertEquals(new NumberValue(-4),
                        interpret("-1 * (4*2/2)"));
    Assert.assertEquals(new NumberValue(0),
                        interpret("-1 * (4*2/2) + 4"));

    Assert.assertEquals(BooleanValue.TRUE,
                        interpret("(-1 * (4*2/2) + 4) == 0"));
    Assert.assertEquals(BooleanValue.FALSE,
                        interpret("(-1 * (4*2/2) + 4) != 0"));

    Assert.assertEquals(BooleanValue.TRUE,
                        interpret("(true || false) && (true || false)"));
    Assert.assertEquals(BooleanValue.TRUE,
                        interpret("true || false && true || false"));
    ...
    Assert.assertEquals(BooleanValue.FALSE,
                        interpret("false || (false && true) || false"));
    Assert.assertEquals(BooleanValue.FALSE,
                        interpret("false || false && true || false"));
    ...
    Assert.assertEquals(BooleanValue.TRUE,
                        interpret("(1-1)>(1*1) || true"));
    Assert.assertEquals(BooleanValue.TRUE,
                        interpret("true || (1-1)>(1*1)"));
  }

  private Object interpret(String input)
      throws IOException
  {
    return Evaluator.evaluate(create(input),
                              null);
  }
