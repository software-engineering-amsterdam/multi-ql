...
public abstract class SemanticMessage
{
      ...
      extends SemanticMessage
  {
    ...
    {
      super(String.format("Type mismatch: '%s' should be of type '%s' but is of type '%s'. ",
                          expr.getSourceText(),
                          expected,
                          actual),
            level,
  ...
  static class OperandTypeMismatch
      extends SemanticMessage
  {
    OperandTypeMismatch(Level level, BinaryExpr expr, ValueType lhsType, ValueType rhsType)
    {
      super(String.format("Type mismatch: operands of %s should be of same type. (lhs='%s', rhs='%s')",
                          expr.getSourceText(),
                          lhsType,
                          rhsType),
            level,
  ...
  static class UndeclaredVariable
      extends SemanticMessage
  {
    UndeclaredVariable(VariableExpr node, Level level)
    {
      super(String.format("Undeclared variable %s, %s",
                          node,
  ...
  static class DuplicateQuestionLabels
      extends SemanticMessage
  {
    DuplicateQuestionLabels(Level level, Question question)
    {
      super(String.format("Duplicate label: %s",
                          question.label()),
            level,
            question.getSourceLocation());
    }
  }

  static class DuplicateQuestionName
      extends SemanticMessage
  {
    DuplicateQuestionName(Level level, Question question)
    {
      super(String.format("Question '%s' has been declared multiple times, but with different types:",
                          question.name()),
            level,
            question.getSourceLocation());
    }
